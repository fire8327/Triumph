    ...
    <div class="fixed top-1/2 -translate-y-1/2 left-1/2 z-[6] transition-all duration-50 w-full px-[15px] flex items-center justify-center text-xl" :class="isServicesFormShow ? '-translate-x-1/2' : 'translate-x-[3000px]'">
        <FormKit @submit="serviceOrder" type="form" form-class="flex flex-col gap-5 w-full max-w-[500px] px-5 py-7 bg-white rounded-[30px] relative" messages-class="hidden" :actions="false">
            ...
            <div class="flex flex-col gap-4 mt-2.5">
                ...
                <FormKit v-model="servicesForm.additional" v-if="servicesForm.service && servicesForm.service != 'Праздник под ключ'" type="checkbox" name="Дополнительные услуги" :options="['Фотограф', 'Ведущий', 'Кейтеринг', 'Украшения', 'Вынос торта']" messages-class="text-[#E9556D] font-Comfortaa text-base mt-2" input-class="px-4 py-1 border border-[#b684b3] focus:outline-none rounded-[20px] mr-5" legend-class="opacity-80 font-Comfortaa" label="Дополнительные услуги"/>
            </div>
